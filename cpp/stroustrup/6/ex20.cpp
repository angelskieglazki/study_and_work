...
{
    no_of_errors++;
    cerr << "error(line:" << curr_line + 1 << "): " << s << endl;
    exit(1);
}
...
double call(istream *input, bool get)
{
    ...
    if (curr_tok != NAME) error("function name expected");

    string def;
    if (!(def = functions[string_value]).size())
        return error("function not defined");
    
    istringstream finput(def);
    return expr(&finput, true);
}
double func(istream *input, bool get)
{
    ...
    if (get_token(input) != ASSIGN) error("= missing");
    string def;
    char ch;
    while (input->get(ch))
    {
        ...
        {
            functions[name] = def;
            cout << "Function " << name << ": " << def << " defined" << endl;
            return 0;
        }
        else
        {
