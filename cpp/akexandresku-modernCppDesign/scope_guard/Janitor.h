...
public:
	Janitor() throw() {}
	template <typename F>
	Janitor(F pFun) : spCmd_(new CmdAdaptor<ScopeGuardImpl0<F>>(pFun)) {}
	template <typename F, typename P1>
	Janitor(F pFun, P1 p1) : spCmd_(new CmdAdaptor<ScopeGuardImpl1<F, P1>>(pFun, p1)) {}
	template <typename F, typename P1, typename P2>
	Janitor(F pFun, P1 p1, P2 p2) : spCmd_(new CmdAdaptor<ScopeGuardImpl2<F, P1, P2>>(pFun, p1, p2)) {}
...
public:
	...
	{
		std::unique_ptr<ICmd_> spTmp(new ObjCmdAdaptor<ObjScopeGuardImpl0<Obj, MemFun>>(obj, memFun));
		spCmd_ = spTmp;
	}
	template <typename Obj, typename MemFun, typename P1>
	ObjJanitor(Obj &obj, MemFun memFun, P1 p1)
	{
		std::unique_ptr<ICmd_> spTmp(new ObjCmdAdaptor<ObjScopeGuardImpl1<Obj, MemFun, P1>>(obj, memFun, p1));
		spCmd_ = spTmp;
	}
};
